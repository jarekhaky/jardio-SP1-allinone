/* Jardio OS v5.6.7.11 - Scheduler persistence, interval scheduling, and copy fixes */
#include <Arduino.h>
#include <SPI.h>
#include <SD.h>
#include <FS.h>
...
bool eventsEquivalentForDay(const ScheduleEvent& a, const ScheduleEvent& b, int targetDay) {
  ...
  if (a.type == "playlist") {
    return a.endTime == b.endTime && a.target == b.target;
  }
  if (a.type == "volume") {
    return a.endTime == b.endTime && a.volume == b.volume;
  }
  if (a.type == "spot") {
    return a.target == b.target && a.repeat == b.repeat && a.volume == b.volume;
  }
  return false;
}
...
void loadScheduleEvents() {
  ...
  } else if (doc.is<JsonObject>() && doc["events"].is<JsonArray>()) {
    arr = doc["events"].as<JsonArray>();
  } else {
    Serial.println("[SCHEDULER] Unsupported events root, expected array or {events:[...]}");
    return;
  }
  ...
  for (JsonVariant item : arr) {
    ...
    if (evt.day < 0 || evt.day > 6) {
      Serial.printf("[SCHEDULER] Skipping event %s with invalid day %d\n", evt.id.c_str(), evt.day);
      skippedCount++;
      index++;
      continue;
    }
    if (evt.type != "playlist" && evt.type != "volume" && evt.type != "spot") {
      Serial.printf("[SCHEDULER] Skipping event %s with invalid type '%s'\n", evt.id.c_str(), evt.type.c_str());
      skippedCount++;
      index++;
      continue;
    }
    ...
    if (!isValidTimeRangeForEvent(evt)) {
      Serial.printf("[SCHEDULER] Skipping event %s with invalid time data start='%s' end='%s'\n", evt.id.c_str(), evt.startTime.c_str(), evt.endTime.c_str());
      skippedCount++;
      index++;
      continue;
    }
...
void saveScheduleEvents() {
  ...
  if (written == 0) {
    Serial.println("[SCHEDULER] Failed to serialize events.json");
    SD.remove(tmpPath);
    return;
  }
  ...
  if (!renamed) {
    Serial.println("[SCHEDULER] Temp rename failed, retrying with copy fallback");
    File src = SD.open(tmpPath, FILE_READ);
    File dst = SD.open(path, FILE_WRITE);
    if (!src || !dst) {
      if (src) src.close();
      if (dst) dst.close();
      Serial.println("[SCHEDULER] Copy fallback failed");
      SD.remove(tmpPath);
      return;
    }
    uint8_t buffer[256];
...
void resumeMusic() {
  ...
  if (scheduledPlaylist > 0) {
    Serial.printf("[AUDIO] Resuming PL%d (vol=%d)\n", scheduledPlaylist, currentMusicVolume);
    isSpotPlaying = false;
    playPlaylist(scheduledPlaylist);
  } else {
...
void processScheduleEvents() {
  ...
  if (activePlaylistNum > 0) {
    if (!isSpotPlaying && (currentPlaylistNum != activePlaylistNum || !isMusicPlaying)) {
      playPlaylist(activePlaylistNum);
      lastActivePlaylistNum = activePlaylistNum;
    }
  } else if (!isSpotPlaying && currentPlaylistNum > 0) {
    audio.stopSong();
    isMusicPlaying = false;
    currentPlaylistNum = 0;
    nowPlayingTrack = "";
  ...
  for (int i = 0; i < scheduleEventCount; i++) {
    ...
    if (activeNow && !evt.active) {
      evt.active = true;
      Serial.printf("[SCHEDULER] Volume active: %s\n", getEventSummary(evt).c_str());
    } else if (!activeNow && evt.active) {
      evt.active = false;
      Serial.printf("[SCHEDULER] Volume ended: %s\n", getEventSummary(evt).c_str());
    }
  }

  if (currentMusicVolume != scheduledMusicVolume) {
    currentMusicVolume = scheduledMusicVolume;
    if (isMusicPlaying && !isSpotPlaying) audio.setVolume(currentMusicVolume);
  ...
  if (activeSpotIdx >= 0 && !audio.isRunning()) {
    ...
    if (evt.pauseUntil > 0) {
      if (nowMs >= evt.pauseUntil) {
        ...
        if (spotFile.length() > 0) {
          ...
          isSpotPlaying = false;
          resumeMusic();
          Serial.printf("[SCHEDULER] Spot%d replay canceled, file missing\n", evt.target);
        }
      }
...
void handleAddEvent() {
  if (!server.hasArg("day") || !server.hasArg("startTime") || !server.hasArg("type")) {
    server.send(400, "text/plain", "Missing required args");
    return;
  }
  if (scheduleEventCount >= 100) {
    server.send(400, "text/plain", "Scheduler full");
    return;
  }
  ...
  evt.startTime = server.arg("startTime");
  evt.endTime = "";
  ...
  if (evt.type == "playlist") {
    if (!server.hasArg("endTime") || !server.hasArg("playlistTarget")) {
      server.send(400, "text/plain", "Playlist requires endTime and playlistTarget");
      return;
    }
    evt.endTime = server.arg("endTime");
    evt.target = server.arg("playlistTarget").toInt();
  } else if (evt.type == "volume") {
    if (!server.hasArg("endTime") || !server.hasArg("volumeValue")) {
      server.send(400, "text/plain", "Volume requires endTime and volumeValue");
      return;
    }
    evt.endTime = server.arg("endTime");
    evt.volume = server.arg("volumeValue").toInt();
  } else if (evt.type == "spot") {
    if (!server.hasArg("spotTarget")) {
      server.send(400, "text/plain", "Spot requires spotTarget");
      return;
    }
    evt.target = server.arg("spotTarget").toInt();
    evt.repeat = server.hasArg("repeat") ? server.arg("repeat").toInt() : 1;
    evt.volume = server.hasArg("spotVolume") ? server.arg("spotVolume").toInt() : defaultSpotVolume;
  } else {
    server.send(400, "text/plain", "Invalid event type");
    return;
  }
  ...
  if (evt.day < 0 || evt.day > 6 || !isValidTimeRangeForEvent(evt)) {
    server.send(400, "text/plain", "Invalid event data");
    return;
  }

  scheduleEvents[scheduleEventCount++] = evt;
  saveScheduleEvents();
  Serial.printf("[API] Event added: %s (%s)\n", evt.id.c_str(), getEventSummary(evt).c_str());
  ...
  if (server.hasArg("id")) {
    ...
    for (int i = 0; i < scheduleEventCount; i++) {
      if (scheduleEvents[i].id == eventId) {
        ...
        if (server.hasArg("day")) updated.day = server.arg("day").toInt();
        if (server.hasArg("startTime")) updated.startTime = server.arg("startTime");
        if (updated.type == "playlist" || updated.type == "volume") {
          if (server.hasArg("endTime")) updated.endTime = server.arg("endTime");
        } else {
          updated.endTime = "";
        }
        if (server.hasArg("target")) updated.target = server.arg("target").toInt();
        if (server.hasArg("repeat")) updated.repeat = constrain(server.arg("repeat").toInt(), 1, 5);
        if (server.hasArg("volume")) updated.volume = constrain(server.arg("volume").toInt(), 0, 21);
...
void handleCopyEvent() {
  if (server.hasArg("id") && server.hasArg("days")) {
    ...
    saveScheduleEvents();
    Serial.printf("[API] Event copied: added=%d skippedSource=%d skippedDuplicates=%d\n", addedCount, skippedSourceDay, skippedDuplicates);
    server.sendHeader("Location", "/scheduler");
    server.send(303);
    return;
  }
